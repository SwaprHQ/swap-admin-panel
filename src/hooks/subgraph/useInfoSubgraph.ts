...
export function useInfoSubgraph() {
  ...
  async function fetchToken(tokenId: string, farming = false) {
    try {
      const {
        data: { tokens },
        error,
      } = await (farming ? farmingClient : dataClient).query({
  ...
  async function fetchInfiniteFarmings() {
    ...
    try {
      ...
      for (const farming of eternalFarmings) {
        const virtualPoolContract = new Contract(farming.virtualPool, VirualPoolABI, provider.getSigner())

        const { reserve0: reward, reserve1: bonusReward } = await virtualPoolContract.rewardReserves()

        let rewardToken = tokenMap[farming.rewardToken]
        if (!rewardToken) {
          rewardToken = await fetchToken(farming.rewardToken, true)
          tokenMap[farming.rewardToken] = rewardToken
        }

        let bonusRewardToken = tokenMap[farming.bonusRewardToken]
        if (!bonusRewardToken) {
          bonusRewardToken = await fetchToken(farming.bonusRewardToken, true)
          tokenMap[farming.bonusRewardToken] = bonusRewardToken
        }
        ...
        _eternalFarmings = [
          ..._eternalFarmings,
          {
            ...
            rewardLeftFor: BigNumber.from(reward)
              .div(Number(farming.rewardRate) ? farming.rewardRate : 1)
              .div(60)
              .toString(),
            bonusRewardLeftFor: BigNumber.from(bonusReward)
              .div(Number(farming.bonusRewardRate) ? farming.bonusRewardRate : 1)
              .div(60)
              .toString(),
          },
        ]
  ...
  async function fetchInfoPools(reload?: boolean) {
    ...
    try {
      ...
      const {
        ...
        query: POOLS_FROM_ADDRESSES(
          undefined,
          topPools.map((el: any) => el.id)
        ),
        fetchPolicy: reload ? 'network-only' : 'cache-first',
      })
  ...
  return {
    ...
    fetchInfiniteFarmings: {
      infiniteFarmings,
      infiniteFarmingsLoading,
      fetchInfiniteFarmingsFn: fetchInfiniteFarmings,
    },
    fetchPool,
  }
}
